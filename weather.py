...
import time
import urllib3 as urllib2 #This module is for Thingspeak(URL Handling Module)
...
def main():
    ...
    while True:
        ...
        finalURL = URL+"&field1=%s&field2=%s" %(humi, temp)+"&field3=%s" %(pressure)+"&field4=%s"%(altitude) # URL which is used to upload the values to website
        s= urllib2.urlopen(finalURL); # This command opens the URL for fetching data
        s.close() # Closing the URL
        print("Humidity : "+str(humi)+"\nTemperature : {0} C".format(temp)+"\nPressure : {0}hPa".format(pressure)+"\nAltitude :"+str(altitude))
        print("----------------------")
        time.sleep(7)
        lcd.clear()
